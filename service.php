...
if (array_key_exists ("url", $_GET) ) {
	...
	if (!$category_l1->load_by_url ($url)) {
...
$meta_title = $category_l1->get_meta_title();
$meta_description = $category_l1->get_meta_description();
...
$serviceImage = $category_l1->get_image();
...
<main class="site-content" data-page="service-category">
  <div class="container container--large inline-hack">
      ...
      <ul class="breadcrumb__nav list--blank">
        ...
        <li class="breadcrumb__item bodyfont bodyfont--smaller"><?= clean_display_string ($category_l1->get_name()) ?></li>
      </ul>
      <h1 class="breadcrumb__header header"><?= clean_display_string ($category_l1->get_name()) ?></h1>
    </nav>

		<section class="main-content main-content--internal">

			<div class="image-text-box mb--90px">
        <div class="image-text-box__box pos--rel">
          ...
          <div class="image-text-box__text no-edge-margin">
            <p class="bodyfont font--black"><?=clean_display_string ($category_l1->get_summary())?></p>
          </div>
        </div>
      </div>
			...
			<div class="block-links no-edge-margin">
				...
				<?php
				$sub_services = $category_l1->get_children();
				if(count ($sub_services) > 0){

					echo '<div class="block-links no-edge-margin">
					<h2 class="block-links__title header font--24px">Our ' . clean_display_string ($category_l1->get_name()) . '</h2>';
					...
					foreach ($sub_services as $sub_service) {
						...
						?>
							...
							<a class="block-links__item no-line pos--rel" href="/service/2/<?=clean_display_string ($sub_service->get_url())?>">
