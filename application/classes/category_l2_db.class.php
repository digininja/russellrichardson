...
class category_l2_db {
	...
	function load_by_url ($url) {
		...
		$query = "SELECT
			...
			FROM category_l2s
			WHERE
				category_l2_url = " . Database::make_sql_value ($url) . " AND 
	...
	function load_if_exists ($id) {
		...
		$query = "SELECT
			...
			FROM category_l2s
			WHERE
				category_l2_id = " . Database::make_sql_value ($id) . " AND 
				category_l2s.category_l2_status <> 'deleted' AND
				1=1
			";
		$res = database::execute ($query);
	...
	function load ($id) {
		if ($this->load_if_exists ($id)) {
			...
			trigger_error ("category_l2_db ($id) not found", E_USER_ERROR);
		}
	}
	function save() {
		if (is_null ($this->_id)) {
			$query = "INSERT INTO category_l2s (
					category_l2_id,
					category_l1_id,
					category_l2_name,
					image_id,
					...
					category_l2_body,
					category_l2_url,
					category_l2_meta_title,
					category_l2_meta_description,
					category_l2_status
				) VALUES (
					null,
					" . Database::make_sql_value ($this->get_category_l1_id()) . ",
			...
			$query = "UPDATE category_l2s SET
								category_l1_id = " . Database::make_sql_value ($this->get_category_l1_id()) . ",
								category_l2_name = " . Database::make_sql_value ($this->get_name()) . ",
								image_id = " . Database::make_sql_value ($this->get_image_id()) . ",
								banner_id = " . Database::make_sql_value ($this->get_banner_id()) . ",
								category_l2_summary = " . Database::make_sql_value ($this->get_summary()) . ",
								category_l2_body = " . Database::make_sql_value ($this->get_body()) . ",
								category_l2_url = " . Database::make_sql_value ($this->get_url()) . ",
								category_l2_meta_title = " . Database::make_sql_value ($this->get_meta_title()) . ",
								category_l2_meta_description = " . Database::make_sql_value ($this->get_meta_description()) . ",
								category_l2_status = " . Database::make_sql_value ($this->get_status()) . "
							WHERE category_l2_id = " . Database::make_sql_value ($this->get_id()) . "
						";
	...
	function populate ($row) {
		$this->_id = intval ($row['category_l2_id']);
		$this->_category_l1_id = intval ($row['category_l1_id']);
		$this->_name = strval ($row['category_l2_name']);
		$this->_image_id = intval ($row['image_id']);
		$this->_banner_id = intval ($row['banner_id']);
		$this->_summary = strval ($row['category_l2_summary']);
		$this->_body = strval ($row['category_l2_body']);
		$this->_url = strval ($row['category_l2_url']);
		$this->_meta_title = strval ($row['category_l2_meta_title']);
		$this->_meta_description = strval ($row['category_l2_meta_description']);
		$this->_status = strval ($row['category_l2_status']);
	}
	...
	function set_category_l1_id ($val) {
		if (is_int ($val)) {
			$this->_category_l1_id = $val;
		} else {
			trigger_error ("Invalid type (" . gettype($val) . "), ($val) passed to set_category_l1_id", E_USER_ERROR);
			return false;
		}
	}
