...
class advice_l1_db {
	...
	function load_by_url ($url) {
		...
		$query = "SELECT
			...
			FROM advice_l1s
			WHERE
				advice_l1_url = " . Database::make_sql_value ($url) . " AND 
	...
	function load_if_exists ($id) {
		...
		$query = "SELECT
			...
			FROM advice_l1s
			WHERE
				advice_l1_id = " . Database::make_sql_value ($id) . " AND 
				advice_l1s.advice_l1_status <> 'deleted' AND
				1=1
			";
		$res = database::execute ($query);
	...
	function load ($id) {
		if ($this->load_if_exists ($id)) {
			...
			trigger_error ("advice_l1_db ($id) not found", E_USER_ERROR);
		}
	}
	function save() {
		if (is_null ($this->_id)) {
			$query = "INSERT INTO advice_l1s (
					advice_l1_id,
					advice_l1_name,
					image_id,
					banner_id,
					advice_l1_summary,
					advice_l1_url,
					advice_l1_meta_title,
					advice_l1_meta_description,
					advice_l1_status
				) VALUES (
					null,
			...
			$query = "UPDATE advice_l1s SET
								advice_l1_name = " . Database::make_sql_value ($this->get_name()) . ",
								image_id = " . Database::make_sql_value ($this->get_image_id()) . ",
								banner_id = " . Database::make_sql_value ($this->get_banner_id()) . ",
								advice_l1_summary = " . Database::make_sql_value ($this->get_summary()) . ",
								advice_l1_url = " . Database::make_sql_value ($this->get_url()) . ",
								advice_l1_meta_title = " . Database::make_sql_value ($this->get_meta_title()) . ",
								advice_l1_meta_description = " . Database::make_sql_value ($this->get_meta_description()) . ",
								advice_l1_status = " . Database::make_sql_value ($this->get_status()) . "
							WHERE advice_l1_id = " . Database::make_sql_value ($this->get_id()) . "
						";
	...
	function populate ($row) {
		$this->_id = intval ($row['advice_l1_id']);
		$this->_name = strval ($row['advice_l1_name']);
		$this->_image_id = intval ($row['image_id']);
		$this->_banner_id = intval ($row['banner_id']);
		$this->_summary = strval ($row['advice_l1_summary']);
		$this->_url = strval ($row['advice_l1_url']);
		$this->_meta_title = strval ($row['advice_l1_meta_title']);
		$this->_meta_description = strval ($row['advice_l1_meta_description']);
		$this->_status = strval ($row['advice_l1_status']);
	}
