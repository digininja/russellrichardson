...
require_once ("application/searches/category_l3.search.php");

class category_l3_list extends base_list {
	private $_id;
	...
	function set_category_l2_id ($val) {
		$this->_category_l2_id = intval ($val);
	}
	...
	function set_onsite ($val) {
		$this->_onsite = strval ($val);
	}
	function set_shredding ($val) {
		$this->_shredding = strval ($val);
	}
	function set_collections ($val) {
		$this->_collections = strval ($val);
	}
	...
	function get_page_details () {
		...
		if (!is_null ($this->_category_l2_id)) {
			$options['category_l2_id'] = $this->_category_l2_id;
		}
		...
		if (!is_null ($this->_onsite)) {
			$options['onsite'] = $this->_onsite;
		}
		if (!is_null ($this->_shredding)) {
			$options['shredding'] = $this->_shredding;
		}
		if (!is_null ($this->_collections)) {
			$options['collections'] = $this->_collections;
		}
		...
		$count = get_category_l3_count ($options);
